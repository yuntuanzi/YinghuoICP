...
try {
    ...
    $totalStmt = $pdo->query("SELECT COUNT(*) FROM icp_records WHERE status = 'approved'");
...
function isSpecialNumber($icpNumber) {
    return preg_match('/(\d)\1{2,}/', $icpNumber);
}
?>
        <main class="flex-grow container mx-auto px-4 py-8">
            <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-pink-400 via-purple-500 to-indigo-600 p-8 md:p-12 mb-8 text-center shadow-lg">
                ...
                <div class="relative z-10">
                    <h1 class="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
                        <span class="inline-block bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full"><?php echo $maintitle; ?></span>
                    </h1>
                    <p class="text-xl md:text-2xl text-white/90 mb-6 font-medium">
                        <span class="inline-block bg-black/20 px-3 py-1 rounded-md"><?php echo $subtitle; ?></span>
                    </p>
                    
                    <div class="flex justify-center space-x-2 mb-4">
                        <?php if(!isset($_SESSION['user_id'])): ?>
                            <a href="note.php" class="inline-block px-3 py-1 bg-pink-200/30 text-pink-100 rounded-full text-sm">✿ 免费加入</a>
                            <a href="search.php" class="inline-block px-3 py-1 bg-purple-200/30 text-purple-100 rounded-full text-sm">✦ 查询备案</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach($icpRecords as $index => $record): 
                    $isSpecialNumber = isSpecialNumber($record['icp_number']);
                ...
                ?>
                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border-2 border-pink-100">
                <div class="bg-gradient-to-r from-pink-400 to-purple-500 p-4 text-white">
                    ...
                    <div class="p-4">
                        ...
                        <div class="space-y-2 mb-3">
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="text-green-700 text-sm">
                                    <?php echo !empty($record['remark']) ? htmlspecialchars($record['remark']) : '网站内容符合相关规定，审核已通过。'; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-gray-700"><?php echo htmlspecialchars($record['update_time']); ?></span>
                        </div>
                    </div>
            ...
            <div class="mt-8 text-center text-gray-600">
                <p>当前共有 <span class="text-pink-600 font-bold"><?php echo $totalRecords; ?></span> 个网站通过备案</p>
                <div class="mt-4">
                    <a href="all-list.php" class="inline-block px-4 py-2 bg-pink-500 text-white rounded-md hover:bg-pink-600 transition-colors">
                        查看所有~喵~
                    </a>
                </div>
            </div>
